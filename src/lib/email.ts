/**
 * Email delivery utility for Ballot Authentication.
 * Supports Resend API out of the box (requires RESEND_API_KEY environment variable).
 */
export async function sendOtpEmail(to: string, code: string): Promise<{ success: boolean; error?: string }> {
  const apiKey = process.env.RESEND_API_KEY;

  if (!apiKey) {
    console.log(`[Ballot Auth - Local Fallback] RESEND_API_KEY not configured. Verification code for ${to}: ${code}`);
    return { success: true };
  }
